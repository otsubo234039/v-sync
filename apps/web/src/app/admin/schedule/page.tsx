    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-[#0F172A] text-slate-800 dark:text-slate-200 font-sans relative overflow-hidden transition-colors">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div 
          ...
          style={{ backgroundImage: `linear-gradient(#22d3ee 1px, transparent 1px), linear-gradient(90deg, #22d3ee 1px, transparent 1px)`, backgroundSize: '40px 40px', maskImage: 'linear-gradient(to bottom, black 20%, transparent 100%)' }} 
        />
      </div>
        ...
        <main className="flex-1 flex flex-col h-screen overflow-hidden">
          ...
          <header className="flex justify-between items-center p-6 shrink-0 bg-white/90 dark:bg-[#0F172A]/90 backdrop-blur z-20 border-b border-slate-200 dark:border-slate-700">
            ...
            <div className="flex gap-2">
              <button onClick={addDemoSchedules} className="bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-4 py-2 rounded-full font-bold transition text-xs">
                🧪 Demo
              </button>
              <button onClick={() => setIsModalOpen(true)} className="bg-cyan-600 hover:bg-cyan-500 text-white px-6 py-2 rounded-full font-bold shadow-lg transition">
                + Add Schedule
              </button>
            </div>
          </header>
          ...
          <div className="flex-1 overflow-hidden relative bg-slate-50 dark:bg-[#0F172A] flex flex-col transition-colors">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar relative">
              ...
              {members.length === 0 && (
                <div className="p-10 text-center text-slate-500 italic">No members registered yet.</div>
              )}
